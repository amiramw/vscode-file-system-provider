...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('memfs.openFile', async (_) => {
			vscode.window.showInformationMessage('Trying to open virtual file "memfs:/file.txt"');
			try {
				await vscode.workspace
					.openTextDocument(vscode.Uri.parse(`memfs:/file.txt`))
					.then(async (doc) => {
				...
				vscode.window.showErrorMessage(
					`Error opening virtual file "memfs:/file.txt". Error was: ${e.message}`
				);
				return;
			}
			vscode.window.showInformationMessage('"memfs:/file.txt" should be open');
		})
	);
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('memfs.init', (_) => {
			...
			memFs.writeFile(vscode.Uri.parse(`memfs:/newnew/file.txt`), Buffer.from('foo'), {
				create: true,
				overwrite: true,
			});
			memFs.writeFile(
